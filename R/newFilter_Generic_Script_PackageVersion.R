...
if(readRawData) {dataSet = xcmsSet2(files=dataPath,profmethod="binlin",profparam=list(profstep=0.01, step=0.01),nSlaves=8,method="centWave",ppm=ppm,peakwidth=peakwidth,snthresh=snthresh,prefilter=prefilter,integrate=integrate,mzdiff=-0.01,fitgauss=T,verbose.columns=T)
...
for(phenoTag in runList)
{
  ...
  if(tTestPairing)
  {
    ...
    cat("\n\nFilling Peaks\n")
    dataSet2 = fillPeaks(dataSet2,"chrom",nSlaves=8,expand.mz=1,expand.rt=1,min.width.mz=0.005,min.width.rt=1) #note: Minwidth for rt is in seconds, scans taken about every .7sec on our machine, exp is factor based (ie 3 = 3X expansion)->1 will do nothing
