group_validationPrep <-
function(xcmsSet2=NULL,pheno=NULL,resultsPath=NULL,intThresh=100, type=c("valid","suspect"),snThresh=5,widthThresh=9,ppmThresh=35)
{
...
#     groupcomp_out <<-groupComposite
    ...
    indx_13C = which((groupComposite[,"iso"]=="13C_6")&((groupComposite[,"maxo_mean"]<intThresh) | (groupComposite[,"sn_mean"]<snThresh) | (groupComposite[,"dppm_mean"]>ppmThresh) | ((groupComposite[,"rtmax_mean"]-groupComposite[,"rtmin_mean"])<widthThresh)))
    ...
    C13Clusters = pairedGroups[which(pairedGroups[ ,"13C_6"] %in% groupComposite[indx,"group"]),"clusterNumber"]
