C13peaks_GroupPairing_old <-
function(xcmsSet2,mzppm=15,mzabs=0.005,rterror=1,resultspath=NULL,pheno=NULL)
{
  ...
  grouped<-lapply(c(1:nrow(C12peak)),function(x)
  {
    ...
    if (l>0)
    {
      rtmin<-as.numeric(min(C12peak[x,"rtmed_M"],plus6[idx,"rtmed_M"],plus6[idx[l],"rtmed_Mplus"])) ##idx[L], calculate min rt for group
      rtmax<-as.numeric(max(C12peak[x,"rtmed_M"],plus6[idx,"rtmed_M"],plus6[idx,"rtmed_Mplus"])) ##idx[L], calculate max rt for group
    ...
    {
      rtmin<-as.numeric(min(C12peak[x,"rtmed_M"],C12peak[x,"rtmed_Mplus"]))
      rtmax<-as.numeric(max(C12peak[x,"rtmed_M"],C12peak[x,"rtmed_Mplus"]))
