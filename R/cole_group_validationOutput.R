cole_group_validationOutput <-
function(xcmsSet=NULL,pheno=NULL,resultsPath=NULL,type=c("valid","suspect"),tTest=F,value=NULL,clusters=NULL,Indx=NULL,Rtindent=2,ppm=NULL,overide=F,tag="NONE")
#   tresults=NULL
{
	#output EIC, MS, and Table
    #Rtindent=2

  check = is.na(xcmsSet@filled[1])
  if(check&!overide)
  {
...
#     cole_group_printEICs(xcmsSet,pairedGroups_matrix,groupComposite,clusters,path,rtrng=24,ppm=ppm,rtindent=Rtindent,output=F) #Default ppm=50,rtrng used to be 5

	#print MS
# 	cat("\n\nGenerating MS for",type,"Clusters in",pheno,"\n")
#     cole_group_printMS(xcmsSet,pairedGroups_matrix,groupComposite,clusters,path,type="multiple",rtindent=Rtindent)
  ...
  gpIdx = match(groupNames,groupnames(xcmsSet))
  matx = cbind(matx,groupval(xcmsSet,method="medret",value=value)[gpIdx, ])
